...
const IPDBookingPage: React.FC = () => {
  const {
    ...
    defaultValues: {
      ...
      address: "",
      uhid: "", // Default UHID to empty string
      relativeName: "",
      relativePhone: "",
      relativeAddress: "",
      date: new Date(),
...
------------------------------------------------------------------ */
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('[data-dropdown="patient-uhid"]') && uhidSuggestions.length > 0) {
        setUhidSuggestions([])
      }
    }
  ...
  }, [patientSuggestions.length, phoneSuggestions.length, uhidSuggestions.length])
  ...
  const handleUhidSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedPatient) {
      setSelectedPatient(null) // Deselect patient if UHID search changes
      reset({
        ...watch(), // Keep current form data
        uhid: "", // Clear UHID
        name: "", // Clear name to allow new input
        phone: "", // Clear phone to allow new input
        gender: null,
        age: 0,
      ...
      setDisplayUHID(""); // Clear displayed UHID
    }
  }
...
------------------------------------------------------------------ */
  ...
  const handleSelectPatient = (patientId: string) => {
    ...
    setValue("name", found.name) // Ensure react-hook-form's name field is updated
    if (found.phone) {
      setPatientPhoneInput(found.phone)
      setValue("phone", found.phone) // Ensure react-hook-form's phone field is updated
    }
    if (found.source === "gautami") {
      ...
      const ageUnitMatch = AgeUnitOptions.find((opt) => opt.value === rec.ageUnit)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(bedsRef, (snapshot) => {
      ...
      const bedList = Object.keys(data)
        ...
        .map((k) => ({
          label: `Bed ${data[k].bedNumber} (${data[k].type || "N/A"})`, // UPDATED: Include bed type in label
          value: k,
        }))
      setBeds(bedList)
      if (bedList.length === 0) {
...
------------------------------------------------------------------ */
    ...
    try {
      ...
      if (selectedPatient) {
        ...
        await update(ref(db, `patients/patientinfo/${patientId}`), {
          ...
          // UHID is not updated here, as it's an existing patient's UHID
        })
      } else {
        patientUHID = await generateNextUHID()
        patientId = patientUHID // For new patients, patientId is the generated UHID
        await set(ref(db, `patients/patientinfo/${patientId}`), {
          ...
          updatedAt: currentTime,
          uhid: patientUHID, // Save UHID here for new patients
        })
      }

      // 3. Prepare split data for IPD admission
      const ipdData = {
        phone: data.phone,
        uhid: patientUHID, // Ensure UHID is part of ipdData
        name: data.name,
      ...
      // Date Key for the node (Mumbai date)
      ...
      // 2. If deposit, push to userbillinginfoipd and its payments sub-node
      if (data.deposit !== undefined && data.deposit >= 0) { // Changed condition to include 0 deposit
        // This is the main billing record for the IPD admission.
        // It stores the *initial* total deposit and payment mode for this admission.
        const billingData = {
          patientId,
          uhid: patientUHID,
          ipdId,
          totalDeposit: data.deposit,
          paymentMode: data.paymentMode?.value || "", // Mode of the initial deposit
          createdAt: currentTime,
        ...
        // This is where individual payment transactions are recorded.
        ...
        await set(paymentRef, {
          ...
          through: data.through?.value || (data.paymentMode?.value === 'cash' ? 'cash' : ''), // ADDED: Save through
          type: "advance", // Could be 'advance', 'partial', 'final', etc.
          amount_type: "advance", // As requested: explicitly mark this payment as a 'deposit'
          createdAt: currentTime,
        })

        // --- Update summary with deposit and payment mode ---
        await updateIpdSummary(dateKey, Number(data.deposit), data.paymentMode?.value || "cash")
      } else {
        // If no deposit (or deposit is undefined/null), update only admissions count in summary
        await runTransaction(ref(db, `summary/ipd/${dateKey}`), (current) => ({
          totalAdmissions: (current?.totalAdmissions || 0) + 1,
          totalDeposit: current?.totalDeposit || 0, // No deposit added
          cash: current?.cash || 0,
          online: current?.online || 0,
        }))
      }
      ...
      const ipdActiveData = {
        ...
        bed: data.bed?.label || "",
        uhid: patientUHID, // Ensure UHID is in active list
      }
      ...
      const patientMessage = `Dear ${data.name}, your IPD admission appointment is confirmed. Your bed: ${
        data.bed?.label || "N/A"
      } in ${data.roomType?.label || "N/A"} has been allocated. Your UHID is ${patientUHID}. Thank you for choosing our hospital.`
      const relativeMessage = ` Dear ${data.relativeName}, this is to inform you that the IPD admission for ${
        data.name
      } (UHID: ${patientUHID}) has been scheduled. The allocated bed is ${
        data.bed?.label || "N/A"
      ...
      reset({
        ...
        doctor: null,
        referDoctor: "",
        admissionType: AdmissionTypeOptions.find((opt) => opt.value === "general") || null, // Reset to default
        admissionSource: AdmissionSourceOptions.find((opt) => opt.value === "opd") || null, // Reset to default
        deposit: 0,
        paymentMode: PaymentModeOptions[0],
        through: null, // Reset through
      })
      setPatientNameInput("")
      setPatientPhoneInput("")
      setSelectedPatient(null)
  ...
  // Update handlePreview to include UHID and Age Unit
  const handlePreview = () => {
    const currentFormData = watch()
    setPreviewData({
      ...currentFormData,
      uhid: selectedPatient?.data.uhid || displayUHID, // Use selected patient's UHID or currently displayed UHID
      ageUnit: currentFormData.ageUnit, // Include ageUnit in preview data
    })
  }
                      ...
                      {uhidSearchTerm.length > 0 && (
                        <button
                          ...
                          onClick={() => {
                            setUhidSearchTerm("")
                            setUhidSuggestions([])
                            setSelectedPatient(null); // Clear selected patient if UHID search term is cleared
                            reset({
                              ...watch(), // Keep current form data
                              uhid: "", // Clear UHID
                              name: "", // Clear name
                      ...
                      {patientNameInput.length > 0 && (
                        <button
                          ...
                          onClick={() => {
                            setPatientNameInput("")
                            setValue("name", "")
                            setSelectedPatient(null) // Clear selected patient if name input is cleared
                            setDisplayUHID("") // Clear UHID display if name is cleared
                            setUhidSearchTerm(""); // Clear UHID search if name is cleared
                            setUhidSuggestions([]); // Clear UHID suggestions
                          }}
                          className="absolute top-3 right-3 text-gray-400 hover:text-gray-600"
                        >
                          <XCircle className="h-5 w-5" />
                      ...
                      {patientPhoneInput.length > 0 && (
                        <button
                          ...
                          onClick={() => {
                            setPatientPhoneInput("")
                            setValue("phone", "")
                            setSelectedPatient(null) // Clear selected patient if phone input is cleared
                            setDisplayUHID("") // Clear UHID display if phone is cleared
                            setUhidSearchTerm(""); // Clear UHID search if phone is cleared
                            setUhidSuggestions([]); // Clear UHID suggestions
                          }}
                          className="absolute top-3 right-3 text-gray-400 hover:text-gray-600"
                        >
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          {...register("age", {
                            ...
                            min: { value: 0, message: "Age must be positive or zero" }, // Changed to allow 0
                          })}
                    ...
                    <div>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Select
                            {...field}
                            options={[{ value: "cash", label: "Cash" }]} // Only cash option
                            placeholder="Cash"
                            styles={customSelectStyles}
                            isDisabled={true} // Disable selection for cash
                          />
                        )}
                      />
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {previewData.age} {previewData.ageUnit?.label}
                  </span>{" "}
                  {/* UPDATED: Display age unit */}
                </p>
                {previewData.address && (
                  <p className="flex justify-between">
              ...
              <div className="space-y-2 col-span-full mt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                  ...
                  {previewData.deposit !== undefined && previewData.deposit >= 0 && ( // Changed condition
                    <p className="flex justify-between">
                      <span className="text-gray-600">Deposit:</span>
                      <span className="font-medium">₹{previewData.deposit}</span>
                    </p>
                  )}
                  {previewData.paymentMode && (
                    <p className="flex justify-between">
                      <span className="text-gray-600">Payment Mode:</span>
                      <span className="font-medium">{previewData.paymentMode.label}</span>
                    </p>
                  )}
                  {previewData.through && (
                    <p className="flex justify-between">
                      <span className="text-gray-600">Through:</span>
                      <span className="font-medium">{previewData.through.label}</span>
                    </p>
                  )}
                </div>
              </div>
                          ...
                          return (
                              ...
                              onClick={() => {
                                ...
                                if (rtOption) {
                                  ...
                                  setTimeout(() => {
                                    ...
                                    const bedOption = {
                                      label: `Bed ${bedInfo.bedNumber} (${bedInfo.type || "N/A"})`, // UPDATED: Include bed type in label
                                      value: bedId,
                                    }
                            ...
                            >
                              ...
                              <span className="text-sm mt-1 font-medium">
                                Bed {bedInfo.bedNumber} ({bedInfo.type || "N/A"})
                              </span>{" "}
                              {/* UPDATED: Display bed type */}
